class SolarPanel {
    ...
    drawControlPanel() {
        ...
        // 繪製自動模式狀態和方向
        const directionText = this.autoTracking ? (this.autoDirection > 0 ? '順時針' : '逆時針') : '';
    ...
    updateStatusPanel() {
        ...
        if (statusPanel) {
            const modeText = this.autoTracking ? '自動' : '手動';
            const directionText = this.autoTracking ? (this.autoDirection > 0 ? '順時針' : '逆時針') : '';
            statusPanel.textContent = `太陽能板: ${modeText}模式 ${directionText} | 角度: ${Math.round(this.angle)}° | 速度: ${this.rotationSpeed.toFixed(2)}`;
        }
